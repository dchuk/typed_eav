...
7. **Phase 7 (Field display label) last** because it is purely additive on top of the shipped Field model and independent of every other phase; it was appended after Phases 1–6 shipped. (The originally-planned read-optimization phase that held this slot was removed on 2026-08-15 — its materialized index would have depended on Phase 3's `on_field_change` event, which is why it sat last.)

### Scope Coverage

**Covers:** scope partitioning extension, pipeline-completion items, event/context contract, opt-in versioning, four new field types (Image / File / Reference / Currency / Percentage), bulk write + schema import-export + CSV mapping + bulk read APIs, and the field display `label` / `display_name` contract.

**Explicitly does NOT cover:**
- MySQL / SQLite portability (Postgres-only is committed; partial unique indexes, GIN, `text_pattern_ops` are PG-specific).
- Read optimization (eager-load helpers, materialized-view index, cache primitives, query-plan helpers) — removed from this milestone 2026-08-15; candidate for a future milestone.
...
| 7 | Field display label (issue #21) | Issue #21 (REQ-05 read optimization removed from milestone) |

## Key Decisions

| Decision | Rationale |
|----------|-----------|
| Postgres-only commitment is binding | The partition tuple (paired partial unique indexes), GIN on jsonb, and `text_pattern_ops` are all PG-specific (as would be any future materialized-view read optimization). Adapter portability is explicitly out of scope. |
| Hook ordering is locked at Phase 3 | Versioning (Phase 4) consumes `on_value_change` / `on_field_change`, and any future materialized index would too. Defining the contract once at Phase 3 prevents refactoring it later. |
...
| Materialized index column generation requires SQL-injection guard rails (carried forward for future read-optimization work) | Field names are user-supplied with minimal constraints today (`RESERVED_NAMES` only excludes Rails STI/timestamp collisions). Restrict to `[A-Za-z0-9_]`, reject reserved SQL identifiers, always quote with `format("%I", name)`. |

## Deferred Ideas

- **Calculated / computed fields** — referenced in plan note line 94. Needs a separate design doc; not part of this roadmap arc.
- **Read optimization** (originally Phase 7; deferred 2026-06-01, removed from milestone 2026-08-15) — eager-load helpers (`Entity.with_all_typed_values`), opt-in materialized value index per `(entity_type, scope, parent_scope)` regenerated via `on_field_change`, `cache_version` / `TypedEAV.cache_key_for` primitives, `TypedEAV.benchmark`. Full scope preserved in git history of ROADMAP.md (§Phase 7 prior to 2026-08-15).
- **Plan-validated explanation layer** for `.explain` (part of read optimization) — keep only if a TypedEAV-specific interpretation adds value over plain AR `.explain`. Otherwise drop at phase start.
