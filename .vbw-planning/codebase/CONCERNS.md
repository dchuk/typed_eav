...
When a host model declares `has_typed_eav scope_method: :tenant_id`, class-level queries (`Contact.where_typed_eav(...)`, `Contact.with_field(...)`) **raise** `TypedEAV::ScopeRequired` if no scope can be resolved (no explicit `scope:` kwarg, no active `with_scope` block, no configured resolver). This is in `TypedEAV::EntityQuery#resolve_scope` (`lib/typed_eav/entity_query.rb`) — moved here from `has_typed_eav.rb` in the 0.3.0 split (ADR-0002); tuple coercion/validation delegates to `TypedEAV::ScopeTuple`.
...
These are honest assessments based on the codebase as it stands at v0.4.0:

- **Bulk APIs shipped (Phase 06)**: `bulk_set_typed_eav_values` (uniform), `bulk_set_typed_eav_values_per_record` (per-record-varying, 0.4.0 G1), and `BulkRead` (N+1-free `typed_eav_hash_for`) now exist. Remaining sharp edge: `BulkWrite.execute_per_record` takes a `Hash<record, vbn>`, so passing two distinct in-memory instances of the same persisted row collapses via Hash-key collision — documented as a gotcha on the per-record API only (`execute` is pair-shaped and immune). CSV import (`CSVMapper`) and schema sync (`SchemaPortability`) are deliberately kept as independent toolbox utilities, not an orchestrated pipeline (ADR-0005).
- **No materialized index for read-heavy use (Phase 07 deferred)**: every query still joins `typed_eav_values` against the host table. For dashboards/analytics on millions of records this will hit a wall. Phase 07 (an optional materialized view registering a second internal subscriber on `EventDispatcher` at slot ≥ 1) was **deferred** during the roadmap reconciliation (commit `fb15a77`). The slot-0 reservation for the versioning subscriber still holds by engine `config.after_initialize` declaration order.
...
- **Pick the right family base first (ADR-0004)**: before writing a new field type, check whether it joins an existing family — string-with-min/max/pattern → subclass `ValidatedString`; comparable-with-min/max bounds → subclass `RangeBounded`; option-set-constrained → include `Optionable`. Only fall back to direct `< Base` for true stubs with no shared surface (Color/Boolean/Json shape). The inheritance-vs-concern rule is locked: inheritance when children share storage, concern when they don't.
- **Adding a single-cell field type**: declare `value_column`, optionally narrow `operators`, implement `cast(raw)` returning the `[casted, invalid?]` tuple, optionally implement `validate_typed_value`. The storage seam + default-operator map now live in `lib/typed_eav/field/typed_storage.rb` (ADR-0001, was `column_mapping.rb`) — the default operator list is keyed by column name, so **don't** add a new value column to the migration without declaring its defaults there.
- **Adding a multi-cell field type**: override **all three** of `self.value_columns` (plural), `self.operator_column(operator)`, and the instance trio `read_value`/`write_value`/`apply_default` (renamed from `apply_default_to` in ADR-0001). Overriding only one creates an asymmetry where reads see the multi-cell shape but writes / defaults populate only one column. Currency is the canonical example (now a normal Field subclass, not a contract-class pair). The versioning subscriber iterates `value_columns` to build snapshots and the `Value#_dispatch_value_change_update` filter iterates them to detect cross-cell changes — both work transparently as long as the trio is paired. Snapshot helpers (`value_changed?`/`before_snapshot`/`after_snapshot`) are concrete and NOT overridable.
- **Adding a new operator**: changes touch `query_builder.rb#filter` (the case dispatch) AND `field/typed_storage.rb` (`DEFAULT_OPERATORS_BY_COLUMN` if it should be on by default for some column type) AND every Field subclass that needs to allow it (or wants an `operator_column` override to route it). For cross-filter/cross-tuple composition (like `include_missing:`, ADR-0006), work at the `FilterQuery` altitude, NOT in `QueryBuilder`. Test coverage in `spec/lib/typed_eav/query_builder_spec.rb` is the matrix to extend.
