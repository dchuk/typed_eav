...
# (`saved_change_to_attribute?(:name)`); a label-only edit must not dispatch
# `:rename` or make consumers rebuild name mappings. Such work would be wasted
# at best and, if a future refactor widened the discriminator to also key on
# `:label`, would corrupt the
