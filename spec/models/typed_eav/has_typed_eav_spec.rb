...
RSpec.describe TypedEAV::HasTypedEAV, type: :model do
  describe "has_typed_eav class method" do
    ...
    it "registers in the global registry" do
      expect(TypedEAV.registry.entity_types).to include("Contact")
    end

    it "stores scope_method" do
      expect(Contact.typed_eav_scope_method).to eq(:tenant_id)
    end

    it "stores type restrictions normalized to strings" do
      # Normalized to strings once at registration so hot paths (type-
      # restriction validation, `typed_eav_attributes=`) don't re-map.
      expect(Product.allowed_typed_eav_types).to eq(%w[text integer decimal boolean])
    end
  end
  ...
  describe ".where_typed_eav", :unscoped do
    let!(:age_field) { create(:integer_field, name: "age", entity_type: "Contact") }
    ...
    it "filters by a single field" do
      results = Contact.where_typed_eav([{ name: "age", op: :gt, value: 28 }])
    ...
    it "supports compact keys (n, op, v)" do
      results = Contact.where_typed_eav([{ n: "city", op: :contains, v: "port" }])
      expect(results).to contain_exactly(alice, charlie)
    end

    it "defaults to :eq when operator is omitted" do
      results = Contact.where_typed_eav([{ name: "city", value: "Seattle" }])
    ...
    it "chains with standard ActiveRecord scopes" do
      results = Contact.where(name: "Alice").where_typed_eav([{ name: "age", op: :gt, value: 20 }])
  ...
  describe "#initialize_typed_values" do
    ...
    it "does not duplicate existing values" do
      TypedEAV::Value.create!(entity: contact, field: field_a).tap do |v|
        v.value = "existing"
        v.save!
      end
  ...
  describe "#typed_eav_attributes=" do
    ...
    it "updates existing values" do
      TypedEAV::Value.create!(entity: contact, field: age_field).tap do |v|
        v.value = 25
        v.save!
      end
    ...
    it "ignores unknown field names" do
      contact.typed_eav_attributes = [{ name: "nonexistent", value: "nope" }]
      contact.save!
  ...
  describe "#typed_eav_hash" do
    ...
    before do
      ...
      TypedEAV::Value.create!(entity: contact, field: bio_field).tap do |v|
        v.value = "Hello"
        v.save!
      end
    end
  ...
  describe "#typed_eav_attributes= with _destroy" do
    let(:contact) { create(:contact) }
  ...
  describe "#typed_eav_attributes= type restrictions" do
    it "skips fields of disallowed types on restricted models" do
      ...
      json_field.update_column(:type, "TypedEAV::Field::Json")
      product = create(:product)
      product.typed_eav_attributes = [{ name: "notes", value: { key: "val" } }]
      product.save!
    ...
    it "allows fields of permitted types" do
      ...
      product = create(:product)
      product.typed_eav_attributes = [{ name: "description", value: "hello" }]
      product.save!
      expect(product.typed_values.count).to eq(1)
    end
  end

  describe "#typed_eav_attributes= with Hash input" do
    let(:contact) { create(:contact) }
  ...
  describe "field definition lifecycle" do
    ...
    it "destroying a select field cascades to options" do
      ...
      expect { field.destroy! }.to change(TypedEAV::Option, :count).by(-option_count)
    end
  end
end
